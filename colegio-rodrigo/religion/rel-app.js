...
function loadProgress(){
  try{
    var s=localStorage.getItem('rel_progress');
...
function saveProgress(){
  try{localStorage.setItem('rel_progress',JSON.stringify({scores:state.scores}));}catch(e){}
}
...
function showResults(){
  ...
  h+='<button class="btn" onclick="showMenu()" style="background:#f6d365">🏠 Menú principal</button>';
  h+='</div></div>';
  c.innerHTML=h;
}
