...
function loadProgress(){
  try{
    var s=localStorage.getItem('mat_progress');
...
function saveProgress(){
  try{localStorage.setItem('mat_progress',JSON.stringify({scores:state.scores}));}catch(e){}
}
...
function showResults(){
  ...
  h+='<button class="btn" onclick="showMenu()" style="background:#11998e">🏠 Menú principal</button>';
  h+='</div></div>';
  c.innerHTML=h;
}
