// Comunicacion Practice App Logic
...
function loadProgress(){
  try{
    var s=localStorage.getItem('com_progress');
...
function saveProgress(){
  try{localStorage.setItem('com_progress',JSON.stringify({scores:state.scores}));}catch(e){}
}
...
function showMenu(){
  ...
  var h='<div class="card"><h2 class="unit-title">📖 Elige un tema para practicar</h2><div class="menu-grid">';
...
function showResults(){
  ...
  h+='<button class="btn" onclick="showMenu()" style="background:#fa709a">🏠 Menú principal</button>';
  h+='</div></div>';
  c.innerHTML=h;
}
