...
function loadProgress(){
  try{
    var s=localStorage.getItem('eng_progress');
...
function saveProgress(){
  try{localStorage.setItem('eng_progress',JSON.stringify({scores:state.scores}));}catch(e){}
}
...
function showResults(){
  ...
  h+='<button class="btn" onclick="showMenu()" style="background:#764ba2">🏠 Main Menu</button>';
  h+='</div></div>';
  c.innerHTML=h;
}
